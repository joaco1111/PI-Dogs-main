...
const { getAllDogs } = require('../controllers/contrllersDog');
const {default: axios } = require('axios');

dogs.use(express.json());
dogs.get('/dogs', async (req, res) => {
    /* RUTAS http://localhost:3001/dogs && http://localhost:3001/dogs/?name=Affenpinscher */
...
dogs.get('/dogs/:idRaza', async (req, res) => {
     /* RUTA http://localhost:3001/dogs/7 */
    try {
